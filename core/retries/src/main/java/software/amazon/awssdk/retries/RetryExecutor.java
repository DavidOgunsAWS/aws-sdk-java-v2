...
@SdkPublicApi
@ThreadSafe
interface RetryExecutor extends Executor {
    /**
     * Create a RetryExecutor based on the {@link StandardRetryStrategy}.
     *
     * <p>Example Usage
     * <pre>
     * RetryExecutor retryExecutor =
     ...
     * </pre>
     */
    static RetryExecutor createStandard(Consumer<StandardRetryStrategy.Builder> configurer) {
        // TODO: implement
        return null;
    }

    /**
     * Create a RetryExecutor based on the {@link AdaptiveRetryStrategy}.
     *
     * <p>Example Usage
     * <pre>
     * RetryExecutor retryExecutor =
     ...
     * </pre>
     */
    static RetryExecutor createAdaptive(Consumer<AdaptiveRetryStrategy.Builder> configurer) {
        // TODO: implement
        return null;
    }
     ...
     * Checked exceptions thrown by the callable are wrapped with a
     * {@link CheckedFailureException}.
     *
     * @throws CheckedFailureException If the provided callable threw a checked exception.
     *                                 The checked exception is preserved as the cause.
     */
    ...
    /**
     * Execute the provided callable asynchronously using the provided executor service, retrying
     * on failure.
     *
     * <p>If the callable succeeds, the future is completed with the result. If the callable or
     * executor fails with a non-retryable exception or if retries are exhausted, the future
     * is completed with the failure exception.
     */
    <T> CompletableFuture<T> executeAsync(Executor executorService, Callable<T> callable);

    /**
     * Execute the provided runnable asynchronously using the provided executor, retrying
     * on failure.
     *
     ...
     * is completed with the failure exception.
     */
    CompletableFuture<Void> executeAsync(Executor executor, Runnable runnable);
